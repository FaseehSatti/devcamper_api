...
exports.addCourse = asyncHandler(async(req,res,next)=>{
    ...
    if(!bootcamp){
        return next(new ErrorResponse(`User ${user.params.id} is not authorized to add a course in bootcamp ${bootcamp._id}`),404); 
    }
    //make sure user is bootcamp owner
    if(bootcamp.user.toString()!==req.user.id && req.user.role !== 'admin'){
        return next(new ErrorResponse(`User ${req.user.id} is not authorized to create this bootcamp`,401))
    }
